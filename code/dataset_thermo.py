...
with open('/home/hongtan/pretrain_single/z_ddmut_dataset/protein_embeddings.pkl', 'rb') as f:
...
class SkempiDataset(Dataset):
    ...
    def __getitem__(self, index):
        ...
        if entry['type'] == 'Forward':
            ...
            data['aa_mut'] = aa_mut
        elif entry['type'] == 'Reverse':
            for mut in entry['mutations']:
                ch_rs_ic = (mut['chain'], mut['resseq'])
                aa_mut[seq_map[ch_rs_ic]] = one_to_index(mut['wt'])
        ...
        if self.transform is not None:
            ...
            if data is None:  # 检查 transform 是否返回 None
                print(f"Skipping entry {entry['mutations']} after transform.")
                return None
