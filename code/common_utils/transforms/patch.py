...
class SelectedRegionFixedSizePatch(object):
    ...
    def __call__(self, data):
        print("Keys in the input dictionary:", list(data.keys()))

        select_flag = (data[self.select_attr] > 0)
        
        if select_flag.sum() == 0:
            print(f"Warning: select_flag is empty for attribute {self.select_attr}, skipping this transform")
            return data  # 或者返回一些默认值，或处理为空选择的情况
        
        pos_CB = _get_CB_positions(data['pos_atoms'], data['mask_atoms'])  # (L, 3)
        print(f"pos_CB shape: {pos_CB.shape}")

        pos_sel = pos_CB[select_flag]  # (S, 3)
        print(f"pos_sel shape: {pos_sel.shape}")
        
        if pos_sel.size(0) == 0:
            print(f"Warning: pos_sel is empty for attribute {self.select_attr}, skipping this transform")
            return data  # 或者返回一些默认值，或处理为空选择的情况
